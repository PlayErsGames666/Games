...
/* Верхняя полоса HUD высотой 61: три ряда мерок (10, 25, 38) плюс строка
   места и работ на 45…59. Была 58, и строка работ упиралась в поле боя. */
let WX0 = 8, WY0 = 61, WX1 = CW - 8, WY1 = CH - 84;   // поле боя внутри холста
...
/* =====================  ЛЮДИ  =====================
   ...
   c — ЦВЕТ ТЕЛА фигуры (её рисует drawFolk), ico — мелкая метка ремесла над
   головой и заодно значок в шапке лавки. Раньше человек весь был одним этим
   значком, и семеро отличались друг от друга только им; теперь первым делом
   в глаза бросается цвет.

   Все семь цветов разные — иначе различать было бы нечего, — и ни один не
   совпадает с цветами королевств (KINGDOMS.c): ими подписано под ногами у
   того же человека, и спорить этим двум краскам не с чем.

   ЯРЧЕ ЗЕМЛИ, а не «под землю». Первый подбор был приглушённый, в тон миру,
   и в деревне это его подвело: на утоптанной площади и на тракте, которые
   сами куда светлее чащи, тёмные тела тускнели и переставали спорить со
   своей же светлой головой — фигура читалась головой на пятне. Прежние
   смайлики были яркими, и людей было видно издали; терять это было бы не
   стилем, а убытком. Поэтому цвета взяты в полсилы яркие: оттенок остался
   землистым, а светлота поднята. */
const NPC_KINDS = {
  trader:  { n: 'Торговец',   ico: '🧔', c: '#c08c46', tabs: ['supply', 'bolt', 'alch'] },
  smith:   { n: 'Кузнец',     ico: '🔨', c: '#808896', tabs: ['weapon', 'armor'] },
  herb:    { n: 'Травница',   ico: '👵', c: '#7fa063', tabs: ['alch', 'supply'] },
  inn:     { n: 'Трактирщик', ico: '🍺', c: '#b25f5f', tabs: ['alch'] },
  bagman:  { n: 'Шорник',     ico: '🎒', c: '#a06b52', tabs: ['bag', 'supply'] },
  guard:   { n: 'Стражник',   ico: '💂', c: '#5c729b' },
  peasant: { n: 'Селянин',    ico: '👨', c: '#9a8b74' },
};
let NPCS = [];
function buildNPCs() {
  NPCS = [];
...
/* Обратного перевода (мир → экран) нет намеренно: мир рисуется сдвигом
   ...
   такое затенение уже разряжали однажды: местная L в drawPawn закрывала
   собой модульную L() — «правила края, где стоим» — на всё тело рисовалки.
   Выстрелить оно не успело, но заряжено было. */
...
/* boltCd тут делает ДВА дела разом, и путать их нельзя. Первое — задержка
   между выстрелами (строка ниже её и читает); её ставят и на неудачном
   нажатии тоже, иначе долбёжка по кнопке с пустым колчаном сыпала бы
   сообщениями каждый кадр. Второе — «арбалет сейчас в руках», по нему пешка
   убирает его с пояса. От пустого колчана арбалет с пояса пропадать не
   должен: выстрела не было. Поэтому неудачу отмечаем отдельно — boltDry. */
function shootBolt() {
  if (P.boltCd > 0 || P.dodge > 0) return;
  P.boltDry = true;                      // пока не доказано обратное — выстрела не было
  ...
  useStack(id, 1);
  P.boltCd = X.cd;
  P.boltDry = false;                     // болт ушёл — вот теперь арбалет в руках
  snd('bolt');
  const a = faceAim();
  shots.push({
...
function hurtPlayer(raw, from) {
  ...
  if (P.quen > 0) {                                    // щит съедает урон, пока не лопнет
    ...
    P.quen -= eat; d -= eat;
    /* ВСПЫШКА — это ЗАЗВЕНЕВШИЙ КУПОЛ, а не отдельное колечко. Радиус берём
       у самого купола: раньше здесь стояла вбитая руками 16, и пока щит был
       кольцом того же радиуса, они совпадали. Купол вырос до QUEN_R = 22, а
       16 осталась — вспышка стала рождаться ВНУТРИ фигуры (след пешки 19),
       вылезать из-под плаща и по дороге пересекать собственный щит.
       Теперь она начинается ровно на ободе и растёт только наружу.

       ПОРЯДОК СЛОЁВ. Купол рисуется ДО пешки, вспышка — вместе с частицами,
       то есть ПОСЛЕ. Так и оставлено намеренно: купол ложится под фигуру
       потому, что иначе его мазки легли бы на плащ и прочлись бы царапинами
...
function castRune(i) {
  ...
  if (R.k === 'igni') {
    ...
    /* Глиф светится краской СВОЕЙ волны. Раньше у Аарда стоял #9fd8ff, у
       Квена #9fe6ff — разница в один канал из двухсот пятидесяти пяти, то
       есть на глаз никакой. А глиф — единственный признак, какой знак пущен,
       первые 0.14 с: до того, как развернётся стихия, отличить их было
       нечем. Теперь каждый носит цвет своего знака и врозь они видны. */
    signCast(a, '#cfeaff');
...
function reset() {
  P = {
    ...
    boltDry: false,           // последнее нажатие на выстрел ушло впустую (пустой колчан,
                              // нет арбалета): задержка идёт, а с пояса арбалет не снимали
...
function bakeCompass(w, h) {
  ...
  g.font = '9px serif'; g.textAlign = 'center'; g.textBaseline = 'middle'; g.fillStyle = '#1a1712';
  for (const k in SPOTS) {
...
/* ЗЕЛЬЕ В СИЛЕ — какого оно цвета на фигуре.
   ...
   показывать нечего.
   Гавна в списке нет по другой причине: пока P.biz > 0, игра рисует
   ВМЕСТО пешки портфель (см. отрисовку игрока), и drawPawn в это время не
   зовут вовсе — обводить нечего. Стой гавно здесь, ободок не появился бы в
   мире ни разу и всплывал бы только в зеркале, где пешка есть всегда.
...
const POTION_SHOWN = [['regen', 'swallow'], ['buffThunder', 'thunder']];
function potionTint() {
...
function pawnState() {
  return {
    ...
    /* Арбалет виден на поясе, пока НЕ стреляет: взвод (boltCd) — это те
       секунды, когда он в руках, а не на боку. Но тот же boltCd ставится и
       на неудачном нажатии — от него арбалет пропадать не должен, выстрела
       не было (см. shootBolt и boltDry). */
    shooting: P.boltCd > 0 && !P.boltDry,
...
/* ТЕЛО ПЕШКИ — приплюснутый поперёк взгляда овал, на который сверху налезает
   голова: те же два круга внахлёст, что и у жителя деревни (drawFolk), только
   этот поворачивается вместе с ведьмаком.

   Раньше тут был плащ-прямоугольник 13…16 в ширину и 17 в длину — то есть
   фигура была ДЛИННЕЕ вдоль взгляда, чем шире поперёк, и читалась палочкой с
   головой. Овал переворачивает это отношение: поперёк — по ширине доспеха,
   вдоль — заметно меньше. Сверху так и выглядит человек: плечи широкие, а
   спереди назад он тонкий.

   Ширину по-прежнему задаёт ВЕС ДОСПЕХА (bw), и она осталась ровно та же —
   тяжёлый шире лёгкого, силуэт по-прежнему говорит, во что ты одет. Длина же
   одна на все доспехи и подобрана так, чтобы тело было площе своей ширины
   даже в САМОМ ЛЁГКОМ: в тяжёлом это заметно, в лёгком фигура почти круглая,
   но нигде не вытянута вдоль взгляда. Иначе «приплюснутый» было бы словом,
   верным только для тяжёлого.

   ЧТО ПРИБИТО К ТЕЛУ. У прямоугольника бока шли отвесно, и всё, что висело на
   краю — наплечники, пластины, арбалет, когти, — попадало на край на ЛЮБОЙ
   высоте. У овала бока заваливаются, и та же высота уводит край внутрь:
   поэтому наплечники переехали на самое широкое место, а пластины, арбалет и
   когти — ближе к середине тела. Мерки на это есть.

   Голова и лицо не тронуты вовсе: они и так были соразмерны, палочкой было
   именно тело. */
const PAWN_BODY = { y: 1.9, ry: 5.8 };                 // середина и полудлина тела; полуширина — bw/2
/* Ободок зелья обводит ВСЮ фигуру, а не одно тело: сверху он обязан накрыть
   макушку (голова: середина -7, радиус 5.2, то есть -12.2), снизу — низ тела
   (1.9 + 5.8 = 7.7). Отсюда и середина, сдвинутая вверх от тела. */
const PAWN_RIM = { y: -2.2, ry: 10 };
function drawPawn(x, y, a, st) {
  ...
  /* Про шаг может быть и не сказано. Не зеркало тому виной — оно зовёт пешку
     через pawnState(), где часы шага есть; неполный st приходит со стенда,
     который рисует фигуру одним нужным ему полем. Math.sin(undefined) — NaN, а холст
  ...
  ctx.fillStyle = 'rgba(0,0,0,.35)';                       // тень под телом
  ctx.beginPath();
  ctx.ellipse(0, PAWN_BODY.y, bw * 0.46, 4.6, 0, 0, 6.3); ctx.fill();
  ...
  /* Мечи за спиной: в руке — только один, второй остаётся торчать. «Назад» —
     это +Y, и клинок обязан выступать ЗА тело, иначе его не видно вовсе.
     Но и не вдвое длиннее самого тела: прежние +6…+14 брались под плащ,
     доходивший до +13, — он прятал клинок почти целиком, и наружу торчало
     остриё. Тело стало короче (кончается на 7.7), и те же числа превратили
     мечи в два длинных уса позади фигуры. Держим прежнюю меру: рукоять под
     плащом, наружу — остриё. */
  ...
  if (steel)  { ctx.strokeStyle = SWORD.steel.c;  ctx.lineWidth = 1.8;
                ctx.beginPath(); ctx.moveTo(-3.5, 4 + bob); ctx.lineTo(-6, 9.5 + bob); ctx.stroke(); }
  if (silver) { ctx.strokeStyle = SWORD.silver.c; ctx.lineWidth = 1.8;
                ctx.beginPath(); ctx.moveTo(3.5, 4 + bob); ctx.lineTo(6, 9.5 + bob); ctx.stroke(); }

  ctx.fillStyle = cloak;                                    // плащ
  ctx.beginPath();
  ctx.ellipse(0, PAWN_BODY.y + bob, bw / 2, PAWN_BODY.ry, 0, 0, 6.3); ctx.fill();
  ...
  if (heavy) {                                              // пластины
    /* Полосы легли ближе к середине тела: у прямоугольного плаща края шли
       отвесно и полоса на любой высоте упиралась в них, а у овала бока
       заваливаются — на прежних +2 и +7 нижняя вылезла бы за плащ и повисла
       в воздухе. */
    ctx.strokeStyle = 'rgba(255,255,255,.16)';
    ctx.beginPath(); ctx.moveTo(-bw / 2 + 1, 0 + bob); ctx.lineTo(bw / 2 - 1, 0 + bob);
    ctx.moveTo(-bw / 2 + 1, 4 + bob); ctx.lineTo(bw / 2 - 1, 4 + bob); ctx.stroke();
  }
  if (heavy || mid) {                                       // наплечники
    /* Плечи сидят на САМОМ ШИРОКОМ месте тела, а не выше него: у овала это
       его середина, и только там наплечник ложится на край плаща, а не
       висит рядом с ним. */
    ctx.fillStyle = 'rgba(0,0,0,.3)';
    ctx.beginPath(); ctx.ellipse(-bw / 2, PAWN_BODY.y + bob, 2.6, 4, 0, 0, 6.3); ctx.fill();
    ctx.beginPath(); ctx.ellipse(bw / 2, PAWN_BODY.y + bob, 2.6, 4, 0, 0, 6.3); ctx.fill();
  }
  ...
  if (st.xbow && !st.shooting) {                            // арбалет на поясе
    ...
    ctx.beginPath(); ctx.moveTo(bw / 2 - 1, 3 + bob); ctx.lineTo(bw / 2 + 4, 5 + bob); ctx.stroke();
  }
  ...
  if (st.mut2) {                                            // когти
    ...
    for (let i = -1; i <= 1; i++) {
      ctx.beginPath(); ctx.moveTo(bw / 2 - 1, PAWN_BODY.y + i * 2.5 + bob);
      ctx.lineTo(bw / 2 + 4, PAWN_BODY.y - 1 + i * 3 + bob); ctx.stroke();
    }
  }
  if (st.potion) {
    ...
    ctx.beginPath();
    ctx.ellipse(0, PAWN_RIM.y + bob, bw / 2 + 2.4, PAWN_RIM.ry, 0, 0, Math.PI * 2); ctx.stroke();
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.beginPath();
      ctx.arc(Math.cos(an) * (bw / 2 + 3), PAWN_RIM.y + bob + Math.sin(an) * (PAWN_RIM.ry - 1.7),
              0.9, 0, Math.PI * 2);
      ctx.fill();
    }
...
/* =====================  ЖИТЕЛЬ ДЕРЕВНИ  =====================
   Человек — ДВА КРУГА ВНАХЛЁСТ: тело пошире и площе, голова поменьше и
   круглая, налезающая на тело сверху. Раньше на его месте стоял смайлик в
   двадцать шагов, и вблизи он читался палкой с головой, а не фигурой.

   ПРОПОРЦИИ решают всё. Сделай голову вдвое меньше тела — и выйдет снеговик,
   а не человек; она берёт почти три четверти от ширины тела, и только тогда
   пара кругов читается плечами и головой. Тело поэтому и приплюснуто: будь
   оно правильным кругом, при такой голове фигура вышла бы башней.

   Голова кладётся ПОСЛЕ тела: её обводка ложится поверх тела, и нахлёст
   виден нахлёстом, а не двумя кругами встык. Ради этого же им и нужна
   обводка — без неё в сумерках два тёмных пятна сливаются в одно.

   РАЗМЕР менять нельзя вольно. Люди уже расставлены по деревням и вытолкнуты
   из дворов (см. buildNPCs), а выталкивают их с зазором в 12 шагов от края
   двора: вырасти фигура шире этого — и она полезет в стену, из которой её
   только что выталкивали. Вниз предел жёстче: на +17 идёт подпись с именем,
   её верх на +12.5, и фигура обязана кончиться выше него.

   ПОКАЧИВАНИЕ — часть размера, а не украшение поверх него. Человек качается
   на полтора шага в обе стороны, и предел внизу надо держать по САМОЙ НИЖНЕЙ
   точке качка, а не по покою. Стоило померить фигуру в покое — и мерка
   отчиталась «12.2, чисто», пока в игре тень заезжала под подпись на 1.2.
   Поэтому качок сюда и передают отдельным доводом: рисовалка знает, что
   качается, а что стоит.

   ТЕНЬ НЕ КАЧАЕТСЯ. Она лежит на земле, и земля под ногами не ходит вверх и
   вниз: качается тело с головой НАД ней. Так же сделано и у пешки. Заодно
   это и есть та поправка, из-за которой тень переставала быть выше подписи.

   Лица нет и не будет: сверху его не видно, а кожа у всех одна — по ней
   голову и узнают как голову. */
const FOLK_BOB = 1.5;                                  // на сколько качается тело с головой
const FOLK_BODY = { y: 4.4, rx: 8.8, ry: 6.2 };        // тело: пошире и приплюснуто
const FOLK_HEAD = { y: -5, r: 6.4 };                   // голова: круглая, внахлёст
const FOLK_SHADOW = { y: 9.4, rx: 7.6, ry: 2.2 };      // тень: на земле, качка не знает
/* Кожа у всех одна — сверху лиц не разглядеть, и выбирать тут нечего. Берём
   её из той же таблицы, откуда её берёт пешка: два одинаковых шестизначных
   числа в разных концах файла разошлись бы при первой же правке палитры. */
const FOLK_SKIN = SKINS.fair.c;
/* Габарит фигуры от её середины: докуда она достаёт вверх, вниз и в стороны.
   Одним радиусом тут не обойтись, и это не придирка: тень плоская и сдвинута
   вниз, так что дальняя от середины её точка лежит НЕ на оси, а наискось, и
   круговой радиус пришлось бы объявить заметно больше того, что фигура
   занимает по-настоящему.

   Числа даны В САМОЙ ДАЛЬНЕЙ ТОЧКЕ КАЧКА, а не в покое: вверх их берёт
   макушка на верхнем качке (11.4 + 1.5), вниз — низ тела на нижнем
   (10.6 + 1.5), и он же обходит неподвижную тень (11.6). Мерить фигуру в
   покое — ровно та ошибка, из-за которой тень заезжала под подпись, а стенд
   об этом молчал.

   Вниз — самое важное из трёх: на +17 идёт подпись с именем, её верх на
   +12.5, и фигура обязана кончиться выше. Вбок предел ставит зазор в 12
   шагов, с которым buildNPCs выталкивает человека из двора; в него мы
   укладываемся с запасом, но расти вширь дальше него уже нельзя.

   Сама рисовалка этих чисел не читает — они ОБЕЩАНИЕ, и держит его стенд:
   он меряет каждый мазок на всех качках и не даёт фигуре ни вырасти молча,
   ни оказаться меньше обещанного. */
const FOLK_BOX = { up: 12.9, down: 12.1, side: 8.8 };
/* x, y — место человека НА ЗЕМЛЕ, bob — на сколько тело с головой поднято
   качком в этот миг. Качок доводом, а не подмешанным в y: тень от него не
   зависит, и слепить их в одно значило бы поднимать над землёй саму землю. */
function drawFolk(x, y, kind, bob) {
  const K = NPC_KINDS[kind];
  const b = bob || 0;
  ctx.fillStyle = 'rgba(0,0,0,.35)';
  ctx.beginPath();
  ctx.ellipse(x, y + FOLK_SHADOW.y, FOLK_SHADOW.rx, FOLK_SHADOW.ry, 0, 0, 6.3); ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,.5)'; ctx.lineWidth = 1.2;
  /* Незнакомое ремесло красим как селянина, а не бросаем без краски: без
     fillStyle тело возьмёт цвет предыдущего мазка — там сейчас чёрная тень. */
  ctx.fillStyle = (K && K.c) || NPC_KINDS.peasant.c;
  ctx.beginPath();
  ctx.ellipse(x, y + FOLK_BODY.y + b, FOLK_BODY.rx, FOLK_BODY.ry, 0, 0, 6.3); ctx.fill(); ctx.stroke();
  ctx.fillStyle = FOLK_SKIN;
  ctx.beginPath(); ctx.arc(x, y + FOLK_HEAD.y + b, FOLK_HEAD.r, 0, 6.3); ctx.fill(); ctx.stroke();
}
...
/* =====================  КУПОЛ КВЕНА  =====================
   ...
   PAWN_R (14) — того самого следа, по которому зеркало вписывает фигуру
...
/* Кольцо «это ты» на земле. Числа вокруг пешки идут лесенкой и обязаны идти
   ею дальше: PAWN_R (14) — докуда достаёт сама фигура, SELF_R (15) — кольцо,
   которое её обводит, не задевая, QUEN_R (22) — купол щита снаружи всего.
   Сдвинь любое из трёх, не глядя на соседей, — и кольцо либо перечеркнёт
   ведьмака, либо слипнется с куполом. Лесенку стережёт стенд. */
const SELF_R = 15;
...
/* =====================  ПЕЧАТЬ ИРДЕНА  =====================
   ...
   толстым и в полную густоту, а всё остальное — паутина, волна, метки —
...
function drawWorld() {
  ...
  txt('U — верстак', BENCH.x, BENCH.y + 24, 11, '#98a2ae', 'center');
  txt('E — доска работ', BOARD.x, BOARD.y + 24, 11, '#be892d', 'center');
  txt('E — облик', MIRROR.x, MIRROR.y + 24, 11, '#c9a0ff', 'center');
  ...
  /* Люди. Ремесло говорит цветом тела, а над головой висит его мелкая метка;
     у кого есть чем торговать, рядом с меткой — монета. Когда меток две, обе
     сдвигаются на полшага, чтобы пара стояла над головой посередине, а не
     свешивалась вправо. Ниже подписываем имя и что жать: без этого человек
  ...
  for (const p of NPCS) {
    ...
    const bob = Math.sin(anim * 2 + p.bob) * FOLK_BOB;
    drawFolk(p.x, p.y, p.k, bob);
  ...
  for (const f of foes) {
    ...
    if (f.name) txt(f.name, f.x, f.y - f.r - 18, 11, '#ffb0a8', 'center');
  }
  ...
  // игрок
  const px = P.x, py = P.y;
  /* «ЭТО ТЫ» — тонкое кольцо на земле под ведьмаком.

     Понадобилось оно с тех пор, как жители деревни стали рисоваться такой же
     фигурой из двух кругов: раньше ведьмака от них отличал сам силуэт, теперь
     — только цвет доспеха да мечи, и в тесной деревне себя можно потерять.

     НА ЗЕМЛЕ, а не над головой: метки ремесла у жителей висят над головой по
     экрану, а ведьмак поворачивается — «над головой» у него всякий раз в
     другой стороне. Кольцо же поворота не знает вовсе.

     САМОЕ ТИХОЕ НА ФИГУРЕ. Вокруг пешки уже спорят двое: горящий контур зверя
     и ободок зелья, и порядок между ними прописан — срыв важнее выпитого.
     Метка стоит ВСЕГДА, а значит говорит о наименее срочном из всего, что тут
     показывают; её и держим тише обоих. Один раз запомнил — дальше узнаёшь
     краем глаза.
...
function drawHUD() {
  ...
  txt('❤ ' + Math.max(0, Math.round(P.hp)) + '/' + Math.round(maxHP()), 14, 14, 10, '#ffd9d0');
  ...
  txt('☠ токсичность ' + Math.round(P.tox) + (P.toxLock > 0 ? ' · 🩸 мутаген ' + Math.ceil(P.toxLock) + 'с' : ''),
      14, 41, 9, P.toxLock > 0 ? '#e69a94' : P.tox > 70 ? '#94e6b6' : '#98a2ae');
  ...
  txt(sw ? SWORD[sw.metal].ico + ' ' + (sw.metal === 'silver' ? 'СЕРЕБРО' : 'СТАЛЬ') : 'без меча', mx + 5, 30, 11, sw ? SWORD[sw.metal].c : '#888');
  txt('Q ⇄', mx + 87, 30, 9, '#98a2ae', 'right');
  txt(sw ? TIERS[sw.tier].n + (sw.ench ? ' ' + ENCH[sw.ench].ico : '') + (sw.oil > 0 ? ' 🧴' + sw.oil : '') : 'Q — сменить', mx + 5, 40, 9, sw && sw.oil > 0 ? '#7fd6a0' : '#98a2ae');

  // золото, вес, болты
  const rx = CW - 10;
  txt('💰 ' + gold, rx, 15, 11, '#d8aa5a', 'right');
  const ld = loadState();                              // не L: L() — это место, и оно тут же рядом
  txt('⚖ ' + carried().toFixed(1) + ' / ' + capacity() + ' кг', rx, 29, 11,
  ...
  /* СТУПЕНЬ И ОПЫТ. Стояли под коробкой меча, в средней колонке, — и строка
     работ ложилась прямо на них: плашка «Лешак 14/14» начинается на 106-м
     пикселе и накрывала полосу опыта на 24 пикселя из 92. В окне это было
     тесно, а в полный экран стало нелепо: полоса перечёркнута, а справа от
     неё полтысячи пикселей пустоты.
     Место опыта — в правой колонке, под болтами: там счёт похода, и ступень
     с золотом и весом одного рода. Средняя колонка теперь кончается на 44-м,
     и строке работ есть куда лечь. */
  {
    const need = xpNeed(P.lvl), w = 92;
    bar(rx - w, 47, w, 6, clamp(P.xp / need, 0, 1), '#be892d');
    txt('⭐ ' + P.lvl + (P.sp > 0 ? '  ·  очков ' + P.sp + ' (K)' : ''), rx - w + 2, 50, 9,
  ...
  if (phase === 'FIGHT' && taken.length) {
    ...
    for (const c of taken) {
      ...
      /* Останавливаемся перед ПРАВОЙ КОЛОНКОЙ, а не перед краем холста: там
         золото, вес, болты и опыт, и наезжать на них нельзя. Ширину колонки
         (92 плюс поля) держим одним числом с самой колонкой. */
      if (lx2 + w2 > CW - 108) break;
      ctx.fillStyle = now ? 'rgba(96,78,36,.55)' : 'rgba(30,28,24,.55)';
      ctx.fillRect(lx2 - 4, 45, w2, 14);
      txt(s2, lx2, 52, 11, now ? '#e6c894' : '#98a2ae');
      lx2 += w2 + 4;
    }
  } else {
    txt(here + ' · контракт ' + (ci + 1) + ' · рекорд: ' + best, CW / 2, 52, 11, '#98a2ae', 'center');
  }
  ...
  // БОЛТ В ЖЁЛОБЕ: клик или B — следующий сорт из тех, что есть в колчане
  {
    ...
    txt(bsel.ico, x + w / 2, y + 11, 14, bhave ? '#dfe4ff' : '#6c7683', 'center');
    txt('B ×' + bhave, x + w / 2, y + 23, 9, bhave ? '#98a2ae' : '#e69a94', 'center');
  }
  ...
  // МУТАЦИЯ: в полном экране кнопок под игрой не видно, а R знают не все
  {
    ...
    txt('🩸 R', x + 4, y + 10, 11, ready ? '#ffb0a8' : '#6c7683');
  ...
  if (msgT > 0) {
    ...
    lines.forEach((l, i) => txt(l, CW / 2, y0 + h / 2 - (lines.length - 1) * 6.5 + i * 13, 11, '#e6c894', 'center'));
    ctx.globalAlpha = 1;
  }
}
...
function drawEquipRow(y) {
  ...
  for (const [nm, it] of slots) {
    ...
    } else txt('пусто', x + 5, y + 20, 11, '#5a616b');
    x += w + 6;
  }
  return y + 42;
}
...
function drawBag() {
  ...
  bagScroll = v.from;
  txt('В сумке: ' + inv.length, 24, y, 11, '#e8d9a8');
  scrollBtns(y, v, () => bagScroll, n => { bagScroll = clamp(n, 0, v.max); });
  y = top;
  if (!inv.length) txt('пусто', 24, y + 6, 11, '#5a616b');
...
function drawBoard() {
  ...
  if (!q) {
    txt('📖 Сюжет пройден. Остались работы с доски.', CW / 2, y + 17, 11, '#be892d', 'center');
    y += 42;
  } else {
  ...
  for (const o of offers) {
    ...
    txt(o.t, 34, y + 14, 11, '#e8d9a8');
    txt('💰 ' + o.gold, CW - 34, y + 14, 11, '#d8aa5a', 'right');
...
function drawMap() {
  ...
  for (const key in SPOTS) {
    ...
    txt(s.n, px(s.x), py(s.y) + 12, 9, '#c2cad2', 'center');
  }
  ...
  for (const p of POWER) {
    ...
    ctx.globalAlpha = 1;
    txt(used ? 'пусто' : p.n, px(p.x), py(p.y) + 12, 9, used ? '#6c7683' : '#c9a0ff', 'center');
  }
  ...
  const S = L();
  txt('Ты в ' + S.ico + ' ' + S.n.toLowerCase() + ' — ' + S.note, CW / 2, 74, 11, '#be892d', 'center');
...
function drawBench() {
  ...
  for (const it of gear.slice(v.from, v.from + v.vis)) {
    ...
    if (maxed) txt('выше некуда', 30, y + 19, 9, '#6c7683');
    else {
      txt('⚒ ' + c.gold + '💰', 30, y + 19, 9, мало.gold ? '#e69a94' : '#98a2ae');
      txt('+ ' + c.mat + STUFF[c.matId].ico, 76, y + 19, 9, мало.mat ? '#e69a94' : '#98a2ae');
...
function drawCard(c, x, y, w, h) {
  ...
  txt(c.mine ? '✓ уже твоё' : c.price + '💰', x + 7, y + h - 9, 11,
...
/* Видимый радиус пешки в её собственных шагах. Через него считается
   увеличение превью в зеркале: не «×4 наугад», а «сколько влезает в рамку».

   Самое дальнее, что фигура рисует, — ОСТРИЁ АРБАЛЕТА на бедре: (12, 5) в
   тяжёлом доспехе, это 13.0 от середины, плюс покачивание — 13.11. Берём 14:
   запаса хватит и на покачивание, и на то, чтобы новая мелочь на фигуре не
   вылезла за рамку молча, но не столько, чтобы ведьмак болтался в рамке.

   Было 19 — под прежний плащ-прямоугольник, у которого мечи уходили до
   (7.9, 14.9). Тело стало овалом и короче, мечи укоротились вслед за ним, и
   19 превратилось в полторы лишних четверти рамки: превью показывало
   ведьмака мельче, чем рамка позволяет. Число меряет стенд и не даёт ему
   ни занизиться, ни снова разойтись с фигурой. */
const PAWN_R = 14;
...
function drawSkills() {
  ...
  BRANCHES.forEach((br, bi) => {
    ...
    for (const k of SKILL_KEYS) {
      ...
      txt(lv + '/' + S.max, x + colW - 5, y + 12, 11, maxed ? '#d8aa5a' : '#98a2ae', 'right');
...
function drawCraft() {
  ...
  txt('Котелок при себе — варить можно хоть посреди болота.', 24, 96, 11, '#98a2ae');
  txt('⚗ Травничество ' + sk('brew') + '/3   ·   ➶ Болторезка ' + sk('fletch') + '/3',
      CW - 24, 96, 11, '#be892d', 'right');
  ...
  let y = top;
  if (!list.length) txt('Пока не умеешь ничего: вложи очко в Травничество или Болторезку (K)', 24, y + 12, 11, '#6c7683');
  for (const r of list.slice(v.from, v.from + v.vis)) {
    ...
    txt(G.ico + ' ' + G.n + (r.out > 1 ? ' ×' + r.out : ''), 32, y + 10, 11, ok ? '#e8d9a8' : '#8a8f96');
...
function render() {
  ...
  if (P.biz > 0) {
    ...
    txt('💼 РЕЖИМ БИЗНЕСМЭНА: ' + P.biz.toFixed(1) + ' с · золото ×3 · ЛКМ — договор', CW / 2, WY0 + 12, 11, '#e6c894', 'center');
  }
  ...
  if (paused && !over && !panel) {
    ...
    txt('P / Esc — продолжить', CW / 2, CH / 2 + 26, 11, '#98a2ae', 'center');
  }
  if (over) {
    ctx.fillStyle = 'rgba(20,6,6,.9)'; ctx.fillRect(0, 0, CW, CH);
    txt('☠ ВЕДЬМАК ПАЛ', CW / 2, CH / 2 - 76, 28, '#d8645a', 'center');
    txt(cause, CW / 2, CH / 2 - 46, 11, '#e6ebf2', 'center');
    txt('Работа сорвана · за лечение отдано ' + downLost + ' крон', CW / 2, CH / 2 - 22, 11, '#c2cad2', 'center');
    ...
    txt('«Заново» — начать поход с нуля', CW / 2, CH / 2 + 82, 11, '#6c7683', 'center');
  }
}
...
if (typeof globalThis !== 'undefined') globalThis.__W = {
  ...
  drawPawn, pawnState, PAWN_BODY, PAWN_RIM,   // rrect наружу не отдаём: это местная кисть рисовалки, мерить в ней нечего
  drawFolk, FOLK_BOX, FOLK_BODY, FOLK_HEAD, FOLK_SHADOW, FOLK_SKIN, FOLK_BOB,
  drawQuenDome, drawYrdenSeal, QUEN_R, QUEN_FACETS, QUEN_CRACKS, QUEN_CRACK_R, SELF_R,
