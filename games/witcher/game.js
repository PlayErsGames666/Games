...
function drawYrdenSeal(Y) {
  ...
  /* Руны едут по самому ободу — они его УКРАШЕНИЕ, а не соперник: потому и
     пишутся вполсилы. В полную густоту на печати идёт только сам обод,
     иначе венец из восьми светлых знаков спорил бы с границей за взгляд.
     Между паутиной (она кончается на 0.81 радиуса) и ободом руны и едут —
     поясок там не пустой, и пустым ему быть незачем. */
  ctx.save(); ctx.translate(Y.x, Y.y); ctx.rotate(rot); ctx.globalAlpha = 0.8;
  for (let i = 0; i < YRDEN_RUNES.length; i++) {
