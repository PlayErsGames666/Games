...
/* =====================  ЖИТЕЛЬ ДЕРЕВНИ  =====================
   ...
   и тень обязана кончиться выше её верха.

   Лица нет и не будет: сверху его не видно, а кожа у всех одна — по ней
   голову и узнают как голову. */
const FOLK_BODY = { y: 5, rx: 8.8, ry: 6.2 };          // тело: пошире и приплюснуто
const FOLK_HEAD = { y: -5, r: 6.4 };                   // голова: круглая, внахлёст
const FOLK_SHADOW = { y: 10, rx: 7.6, ry: 2.2 };       // тень: кончается выше подписи
...
/* Габарит фигуры от её середины: докуда она достаёт вверх, вниз и в стороны.
   ...
   +12.5, и тень обязана кончиться выше. Вбок предел ставит зазор в 12 шагов,
   с которым buildNPCs выталкивает человека из двора; в него мы укладываемся
   с запасом, но расти вширь дальше него уже нельзя.

   Сама рисовалка этих чисел не читает — они ОБЕЩАНИЕ, и держит его стенд:
   он меряет каждый мазок и не даёт фигуре ни вырасти молча, ни оказаться
   меньше обещанного. */
const FOLK_BOX = { up: 11.4, down: 12.2, side: 8.8 };
function drawFolk(x, y, kind) {
  ...
  ctx.beginPath();
  ctx.ellipse(x, y + FOLK_BODY.y, FOLK_BODY.rx, FOLK_BODY.ry, 0, 0, 6.3); ctx.fill(); ctx.stroke();
  ctx.fillStyle = FOLK_SKIN;
  ctx.beginPath(); ctx.arc(x, y + FOLK_HEAD.y, FOLK_HEAD.r, 0, 6.3); ctx.fill(); ctx.stroke();
}
...
if (typeof globalThis !== 'undefined') globalThis.__W = {
  ...
  drawFolk, FOLK_BOX, FOLK_BODY, FOLK_HEAD, FOLK_SHADOW, FOLK_SKIN,
