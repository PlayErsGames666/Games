/* Облик ведьмака: таблицы, запись, зеркало, панель.
   ...
   В хвосте — облик ЖИТЕЛЕЙ деревни: у них своя рисовалка из двух кругов. */
'use strict';
...
head('Ходьба покачивает фигуру, а стояние — нет');
{
  /* Покачивание — единственное, чем ходьба видна на фигуре. Обнулить его
     ...
     что взяться. Берёмся за то, ГДЕ фигура нарисована.

     Одних точек пути мало, и это доказано порчами: голова, глаза и
     наплечники нарисованы дугой и овалом, в traces их нет вовсе, и снятое
     у каждого из них покачивание не роняло ни одной мерки — при том что
     мерка ниже называется «сдвинулась ВСЯ фигура». Поэтому берём и точки
     пути, и дуги: вместе это и есть вся фигура.

     Тень — единственное, чему покачиваться НЕЛЬЗЯ: она лежит на земле, а
     не на ведьмаке, и в коде рисуется без bob намеренно. Отделяем её по
     собственной краске и меряем отдельной строкой — иначе отбор «всё, кроме
     тени» сам стал бы дырой: перекрасил мазок в тень — и он выпал из счёта. */
  const SHADOW = 'rgba(0,0,0,.35)';                     // тень пешки, её единственная краска
  const partsOf = w => {
    const st = { armor: 'heavy', look: W.LOOK_DEF, walk: w };
    const path = traces(() => W.drawPawn(0, 0, 0, st)).map(e => e.y);
    const round = arcs(() => W.drawPawn(0, 0, 0, st));
    return { fig: path.concat(round.filter(a => a.c !== SHADOW).map(a => a.y)),
             shadow: round.filter(a => a.c === SHADOW).map(a => a.y) };
  };
  const yOf = w => partsOf(w).fig;
  ...
  note('в счёте точек фигуры: ' + base.length + ' (пути и дуги вместе)');
  const sh0 = partsOf(0).shadow, sh1 = partsOf(PERIOD / 4).shadow;
  ok(sh0.length > 0 && sh0.every((y, i) => y === sh1[i]),
     'а тень на земле стоит на месте — покачивается ведьмак, не земля под ним');
...
/* =====================  ЖИТЕЛИ ДЕРЕВНИ  =====================
   ...
   из которой её выталкивали, а цвета ремёсел — совпасть, и тогда различать
   станет нечего. Меряем по мазкам: стенд видит каждую дугу с её краской. */
head('Житель — два круга внахлёст, а не палка');
{
  const K = W.NPC_KINDS.smith;
  const a = arcs(() => W.drawFolk(0, 0, 'smith'));
  note('мазков в фигуре: ' + a.map(m => m.k + ' ' + m.c).join(' · '));

  const body = a.find(m => m.k === 'fill' && m.c === K.c);
  const head_ = a.find(m => m.k === 'fill' && m.c === W.FOLK_SKIN);
  ok(!!body, 'тело залито цветом ремесла (' + K.c + ')');
  ok(!!head_, 'голова залита цветом кожи (' + W.FOLK_SKIN + ')');
  ok(!!body && !!head_ && body.rx > head_.rx,
     'тело шире головы' + (body && head_ ? ' (' + body.rx + ' против ' + head_.rx + ')' : ''));
  ok(!!body && !!head_ && head_.y < body.y, 'голова выше тела');
  /* Голова не должна быть мелкой: сделай её вдвое меньше тела — и выйдет
     снеговик, а не человек. Держим не меньше двух третей ширины тела. */
  if (body && head_) {
    const k = head_.rx / body.rx;
    note('голова к телу по ширине: ' + k.toFixed(2));
    ok(k >= 0.66, 'голова крупная, фигура читается человеком, а не снеговиком');
  }
  /* Нахлёст: низ головы обязан зайти НИЖЕ верха тела. Встань они встык — и
     это уже не «два круга друг в друге». Меряем по ВЕРТИКАЛЬНОЙ полуоси:
     тело приплюснуто, и его ширина тут ни при чём. */
  if (body && head_) {
    const gap = (body.y - body.ry) - (head_.y + head_.ry);
    note('низ головы ниже верха тела на ' + (-gap).toFixed(1));
    ok(gap < 0, 'круги перекрываются, а не стоят встык');
  }
  /* Голова кладётся ПОСЛЕ тела — иначе её обводка уйдёт под тело и нахлёст
     перестанет читаться как нахлёст. */
  ok(!!body && !!head_ && a.indexOf(body) < a.indexOf(head_),
     'голова нарисована поверх тела');
...
{
  const keys = Object.keys(W.NPC_KINDS);
  const cols = keys.map(k => W.NPC_KINDS[k].c);
  note(keys.map((k, i) => W.NPC_KINDS[k].n + ' ' + cols[i]).join(' · '));
  ok(cols.every(c => !!c), 'цвет задан каждому ремеслу');
  ok(new Set(cols).size === cols.length, 'все ' + cols.length + ' цветов разные');
  const kd = Object.keys(W.KINGDOMS).map(k => W.KINGDOMS[k].c);
  const clash = cols.filter(c => kd.indexOf(c) >= 0);
  ok(clash.length === 0, 'ни один не спорит с цветом королевства в подписи' +
     (clash.length ? ': ' + clash.join(', ') : ''));
}

head('Люди в деревне нарисованы фигурой, а не смайликом');
{
  ...
  /* Голову ищем ПО МЕСТУ, а не по одной краске: цвет кожи у жителя тот же,
     что у ведьмака (FOLK_SKIN и есть SKINS.fair), и пешка сама подмешивает
     в кадр лишнюю голову. Разброс по высоте — это покачивание bob (±1.5). */
  ...
  ok(lost.length === 0, 'у каждого человека нарисована голова на его месте' +
     (lost.length ? ': не нашлось у ' + lost.map(p => p.n).join(', ') : ''));
  /* Значки пекутся ОДИН раз и дальше кладутся готовой картинкой, а стенд
     видит только печать. Кадром выше мы их уже испекли — не опустошив печь,
     мерили бы пустоту, и «метки мелкие» вышло бы правдой ни на чём. */
  peek('icoCache').clear();
  const ms = frameMarksAll().filter(m => folk.some(p => p.ico === m.s));
  ok(ms.length > 0, 'метки ремесла в кадре есть');
  ok(ms.every(m => m.size <= 10),
     'прежние смайлики остались только мелкими метками' +
     (ms.length ? ' (кегль ' + ms.map(m => m.size).join(', ') + ')' : ''));
}
