  ...
  return new Proxy({}, {
    get(t, k) {
      ...
      if (k === 'arc') return (x, y, r, a0, a1, ccw) => {
        ...
        // у круга наклон бессмыслен, но поле кладём: мерке проще не гадать
        pathBuf.push({ x, y, r, rx: r, ry: r, tilt: 0, a0, a1, ccw: !!ccw,
      ...
      /* Овал кладём в тот же путь, что и дугу, с радиусом по БОЛЬШЕЙ оси:
         ...
         НАКЛОН кладём туда же, и это не для полноты. rx и ry живут в осях
         САМОГО овала, а x и y — в мировых, и у наклонённого овала это разные
         оси: возьми мерка ry за высоту у частицы крови (её рисуют с
         поворотом p.a) — и посчитает не то, ничем себя не выдав. Пока такой
         мерки нет, но ry как «высоту» здесь теперь и советуют, так что
         ловушку надо было либо убрать, либо сделать видимой. Сделали
         видимой: у прямого овала tilt равен нулю, и мерка, которой наклон
         безразличен, может честно это проверить. */
      if (k === 'ellipse') return (x, y, rx, ry, tilt, a0, a1, ccw) => {
        nanArgs(k, [x, y, rx, ry, tilt, a0, a1]);
        const r = Math.max(rx, ry);
        pathBuf.push({ x, y, r, rx, ry, tilt: tilt || 0, a0, a1, ccw: !!ccw, oval: true,
                       sx: scrX(x), sy: scrY(y), rs: r * ttop().sx });
      };
