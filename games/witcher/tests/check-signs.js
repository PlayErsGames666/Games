...
head('Ирден: нарисованный обод и есть граница замедления');
{
  ...
  /* Обод — ГРАНИЦА, и это должно быть видно с одного взгляда: он самый
     ...
     мажет по холсту, — надписи в том числе. Перебирать одни дуги здесь
     значило бы не заметить восьми светлых рун поверх них: мерка называлась
     бы громче, чем меряет. Ради этого обвязка и пишет теперь fillText. */
  ...
  ok(loud.length === 1 && loud[0].k === 'stroke' && loud[0].c === '#c496ff',
     'в полную густоту на печати идёт ТОЛЬКО обод — ни нить, ни волна, ни руна с ним не спорят');
  const runes = all.filter(m => m.k === 'text');
  ok(runes.length === W.YRDEN_RUNES.length && runes.every(m => ink(m) < ink(loud[0] || rim)),
     'руны пишутся вполсилы: они украшение обода, а не соперник ему (' +
     (runes[0] ? ink(runes[0]).toFixed(2) : '—') + ' против единицы у обода)');
  ...
  note('паутина занимает от ' + webMin.toFixed(1) + ' до ' + webMax.toFixed(1) +
       ' при границе ' + R.toFixed(1) + ', поясок между нитями и ободом ' + (R - webMax).toFixed(1));
  /* Между паутиной и ободом должен остаться ПОЯСОК БЕЗ НИТЕЙ — иначе нити
     упрутся в границу, и глаз перестанет отличать «край ловушки» от «ещё
     одна нить». Пустым этот поясок не бывает и быть не должен: по нему
     едут руны — но они украшение самого обода и написаны вполсилы (см.
     мерку густоты выше), а нити — нет.
     Мерить «webMax < R» мало: нить, дотянувшаяся ровно до обода, прошла бы
     такую мерку на последнем знаке после запятой. */
  ok(R - webMax >= R * 0.1, 'между нитями и ободом есть поясок — граница не тонет в паутине');
