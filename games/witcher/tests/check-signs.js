...
head('Потолок частиц');
{
  ...
  /* --- ГРАНИЦА ЗНАКА ВЫТЕСНЕНИЮ НЕ ПОДЛЕЖИТ ---

     По возрасту она в самом низу стопки: дуга рождается вместе с выбросом,
     а поверх неё тут же сыплется шесть десятков языков, искр и дыма. Значит
     при сработавшем потолке уходила бы ровно она — единственное, что
     ОБЕЩАЕТ игроку «досюда достанет». Ставим её САМОЙ СТАРОЙ нарочно и
  ...
  for (let i = 0; i < HITS; i++) W.hurtFoe(f, 1, 'sword');
  const rest = W.getParts();
  note('после ' + (perHit * HITS) + ' заказанных частиц в массиве ' + rest.length +
       ', границ уцелело ' + bounds.filter(b => rest.indexOf(b) >= 0).length + ' из ' + bounds.length);
  ok(rest.length === W.PART_CAP, 'потолок держит и с защищёнными: ' + rest.length);
  ok(rest.indexOf(bounds[0]) >= 0, 'граница знака пережила вытеснение, хотя была самой старой');
  ok(rest.indexOf(bounds[1]) >= 0, 'и выжженное пятно тоже — это след той же зоны');

  /* Но НЕ несгораемая: если защищённых окажется больше потолка, кадр важнее
     обещания — режем и их, по возрасту. Иначе массив рос бы без предела. */
  W.getParts().length = 0;
  const many = [];
  ...
  W.hurtFoe(f, 1, 'sword');
  const after = W.getParts();
  note('одни границы: было ' + many.length + ', осталось ' + after.length + ' при потолке ' + W.PART_CAP);
  ok(after.length === W.PART_CAP, 'потолок не пробит даже сплошными границами: ' + after.length);
  ok(after.indexOf(many[0]) < 0 && after.indexOf(many[many.length - 1]) >= 0,
     'и резались они по возрасту: самой старой нет, самая свежая на месте');
}
...
head('Квен копит трещины, но не бесконечно');
{
  ...
  const seen = [];
  let first = null;
  const HITS = W.QUEN_CRACKS * 2 + 2;                  // заведомо больше потолка, но считаем от него
  ...
  note('трещин после каждого из ' + HITS + ' ударов: ' + seen.join(' '));
  ...
  ok(seen[last] === seen[last - 1] && seen[last] < HITS,
     'счёт упёрся в потолок: ' + seen[last] + ' трещин при ' + HITS + ' ударах');
  ok(seen[last] >= 3, 'но потолок не в одну-две: по трещинам должно быть видно, сколько щит уже съел');
...
head('Купол Квена — гранёный, а не круг');
{
  ...
  /* Нарисовано ровно столько граней, сколько ОБЪЯВЛЕНО. Само число (восемь)
     мерке не подчиняется — спросить «их восемь?» у той же восьмёрки значит
     написать мерку, которая не упадёт никогда; зато разъехаться рисунку с
     собственной ручкой нельзя. Границы вкуса стережём отдельно: из четырёх
     граней выйдет ромб, из тридцати — круг, а обещан гранёный купол. */
  ok(mv.length === W.QUEN_FACETS,
     'граней нарисовано столько же, сколько объявлено: ' + mv.length + ' против ' + W.QUEN_FACETS);
  ok(W.QUEN_FACETS >= 6 && W.QUEN_FACETS <= 12,
     'и купол остался гранёным, а не ромбом и не кругом: ' + W.QUEN_FACETS + ' граней');
...
head('Купол не закрывает ведьмака');
{
  ...
  /* Обод и вылет лучей сверяем С ОБЪЯВЛЕННЫМИ ручками, а не с числами в
     мерке: разъедься рисунок с ними — и все рассуждения о том, что купол
     снаружи пешки, повиснут в воздухе. */
  const domeR = Math.max.apply(null, t.slice(0, W.QUEN_FACETS * 2).map(e => Math.hypot(e.x, e.y)));
  ok(Math.abs(domeR - W.QUEN_R) < 1e-9,
     'вершины граней лежат ровно на QUEN_R: ' + domeR.toFixed(3) + ' против ' + W.QUEN_R);
  ok(Math.abs(far - Math.max.apply(null, W.QUEN_CRACK_R)) < 1e-9,
     'дальше всего от середины уходит луч трещины, ровно на QUEN_CRACK_R: ' +
     far.toFixed(2) + ' против ' + Math.max.apply(null, W.QUEN_CRACK_R));
  ok(near >= W.PAWN_R,
...
head('Трещина показывает, ОТКУДА прилетело');
{
  ...
  for (const want of [0, 1.9, -2.6]) {
    ...
    const t = traces(() => W.drawQuenDome(0, 0));
    let wide = 0, cnt = 0;
    for (const e of t.slice(W.QUEN_FACETS * 2)) {     // первые точки — сами грани, по две на грань
      cnt++;
