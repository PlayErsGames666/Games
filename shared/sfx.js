...
(function (global) {
  ...
  /* Часы у ограничителя СВОИ и всегда одни и те же. Раньше он смотрел на
     ctx.currentTime, а до появления звукового движка — на Date.now(): первый
     же звук, сыгранный до касания страницы, записывал полтора миллиарда, и
     дальше разность выходила отрицательной — этот звук глох навсегда.
     Заодно Date.now() идёт и когда контекст усыплён, а currentTime стоит. */
  const last = {};
