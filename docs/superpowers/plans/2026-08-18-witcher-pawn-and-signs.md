...
- [ ] **Шаг 2: Прогнать — записать, что вышло**

Запустить: `node games/witcher/tests/check-look.js`

Эти две проверки на СТАРОЙ отрисовке пройдут: два круга не падают ни при каком облике. Это ожидаемо — они стерегут не «пешка есть», а «пешка не падает ни на одном значении таблиц», и смысл появится, когда пешка появится. Поэтому проверка тут не падающая, а мутационная: её честность подтверждается шагом 8, а не сейчас. Записать вывод и идти дальше.
...
- [ ] **Шаг 8: Убедиться, что проверка облика не пустая**

Временно сломать `drawPawn`: в строке, где берётся цвет волос, заменить
`(HAIR_C[look.hairC] || HAIR_C.white).c` на `HAIR_C[look.hairC].c` — то есть
убрать защиту от незнакомого ключа. Затем в проверке временно подсунуть чужой
ключ: в `check-look.js` в разделе «Рисуется любой облик из таблиц» после
`W.setLook(L);` дописать `if (F.k === 'hairC') W.setLook(Object.assign({}, L, { hairC: 'нетакого' }));`

Запустить: `node games/witcher/tests/check-look.js`
Ожидаем: `БАГ падение на hairC=...` — проверка ловит.

Вернуть обе правки обратно, прогнать снова — чисто. Проверка, которая не падает
на подсунутой поломке, ничего не стоит.
...
### Задача 8: Игни и Аард — вместе с глифом и границей

Глиф и граница — общая основа всех четырёх знаков, но приезжают они вместе с первым, кто их зовёт. Отдельной задачей они были бы кодом, который никто не вызывает, а это брак, сколько бы он ни прожил.

**Files:**
- Modify: `games/witcher/game.js` — рядом с `castRune`; ветки `igni` и `aard` в `castRune` (строки 1822–1831); отрисовка частиц (около 3414)
- Modify: `games/witcher/tests/check-signs.js`

**Interfaces:**
- Consumes: `addPart` (задача 7).
- Produces: `signCast(a, col)`; роды частиц `glyph`, `edge`, `flame`, `spark`, `smoke`, `scorch`, `wave`, `dust`.
...
- [ ] **Шаг 2: Нарисовать глиф и границу**
...
- [ ] **Шаг 5: Нарисовать остальные роды**
...
- [ ] **Шаг 6: Прогнать сетку зон — главное подтверждение**
...
Открыть `games/witcher/index.html` двойным щелчком, взять работу, пустить Игни и Аард в тварей и в пустоту.
...
head('Квен копит трещины, но не бесконечно');
{
  ...
  for (let i = 0; i < 12; i++) { P.quen = 500; P.inv = 0; P.dodge = 0; W.hurtPlayer(5, 'проверка'); }
...
Обрати внимание на `P.inv = 0; P.dodge = 0;` внутри цикла: `hurtPlayer` первой же строкой отбрасывает удар, если ведьмак в неуязвимости или в уклонении, а после первого попадания неуязвимость включается. Без сброса двенадцать ударов превратились бы в один, и проверка ничего бы не проверила.
...
В `hurtPlayer`, там где щит поглощает урон (около строки 1794), ДО строки `if (P.quen <= 0) message('🛡 Квен разбит');` дописать:
...
### Задача 10: Справка и README стенда
...
| Не пекём, замерить потом | 3 шаг 10, 6 шаг 10, 8 шаг 9 |
...
| Панель облика | 6 |
| Общее правило знаков: глиф и граница | 8 (вместе с первым, кто их зовёт) |
| Игни | 8 |
| Аард | 8 |
| Квен | 9 |
| Ирден | 9 |
| Потолок частиц | 7 |
| Проверки: `check-signs`, `check-look`, правки в трёх файлах | 1, 2, 5, 6, 7, 9, 10 |
| Обновить README стенда | 10 |
